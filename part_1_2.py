...
# Générer les requêtes INSERT INTO pour les 100 utilisateurs
insert_user_queries = []
for _ in range(100):
    firstname = fake.first_name()
    lastname = fake.last_name()
    email = fake.email()
    username = fake.user_name() + str(random.randint(100, 999))  # Assure l'unicité du nom d'utilisateur
...
# Générer les requêtes INSERT INTO pour les 85 vérifications d'email
...
for _ in range(1000):
    user_id = random.randint(1, 85)  # Choose a user with email verification
    verification_date = email_verification_dates[user_id - 1]
    connected_at = fake.date_time_between_dates(datetime_start=datetime.datetime(2023, 1, 1),
                                                datetime_end=datetime.datetime(2023, 6, 30))
